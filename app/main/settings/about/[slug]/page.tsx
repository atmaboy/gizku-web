...
export default function LegalDocumentDetailPage() {
  ...
  return (
    <div className="flex flex-col min-h-0 flex-1">
      <ScreenHeader title={title ?? 'Dokumen'} onBack={() => router.push('/main/settings/about')} />
      <div className="flex-1 overflow-auto px-4 pt-5 pb-10">
        {notFound ? (
          <p className="text-sm text-secondary text-center mt-10">Dokumen tidak ditemukan.</p>
        ) : bodyHtml ? (
          <div className="legal-doc-body" dangerouslySetInnerHTML={{ __html: bodyHtml }} />
        ) : (
          <p className="text-sm text-secondary text-center mt-10">Memuat…</p>
        )}
      </div>
    </div>
  )
