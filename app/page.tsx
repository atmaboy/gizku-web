...
function NutritionRing({ size = 120 }: { size?: number }) {
  ...
  const segments = [
    { pct: 0.45, color: '#2ECC71', label: 'Karbo' },
    { pct: 0.30, color: '#3B82F6', label: 'Protein' },
    { pct: 0.25, color: '#F59E0B', label: 'Lemak' },
  ]
  let offset = 0
  return (
...
function PhoneMockup() {
  ...
  return (
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ filter: 'drop-shadow(0 20px 40px rgba(0,0,0,0.18))' }}>
      ...
      {/* Food photo area */}
      <rect x="8" y="66" width={W-16} height="90" fill="#F0FDF4"/>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 480px) {
          ...
          .gk-cta-perks { align-items: flex-start; padding-left: 16px; }
        }
